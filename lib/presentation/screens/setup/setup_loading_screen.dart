...
class SetupLoadingScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<SetupLoadingScreen> createState() => _SetupLoadingScreenState();
}
...
class _SetupLoadingScreenState extends ConsumerState<SetupLoadingScreen>
  ...
  final List<SetupStep> _steps = [
    SetupStep('Verificando conexão...', 0.2),
    SetupStep('Baixando alimentos...', 0.6),
    SetupStep('Preparando dados...', 0.8),
  ...
  Future<void> _startSetup() async {
    try {
      ...
      // Passo 2: Sincronizar alimentos da API (CRÍTICO para offline-first)
      await _updateStep(1);
      final alimentoRepository = ref.read(alimentoRepositoryProvider);
      final syncResult = await alimentoRepository.syncAlimentosFromAPI();

      syncResult.fold(
        (failure) {
          throw Exception('Erro ao baixar alimentos: ${failure.message}');
        },
        (count) {
          debugPrint('✅ $count alimentos sincronizados com sucesso!');
        },
      );
